...
function test_merge_stat(o, y1, y2)
    o2 = deepcopy(o)
    fit!(o, y1)
    fit!(o2, y2)
    merge!(o, o2)
...
function test_merge_value(o, y1, y2, compare = ≈; kw...)
    o2 = copy(o)
    fit!(o, y1)
    fit!(o2, y2)
    merge!(o, o2)
...
#     test_merge_value(CountMap(SortedDict{Bool, Int}()), rand(Bool, 100), rand(Bool, 100), ==)
#     test_merge_value(CountMap(SortedDict{Bool, Int}()), trues(100), falses(100), ==)
#     test_merge_value(CountMap(SortedDict{Int, Int}()), rand(1:4, 100), rand(5:123, 50), ==)
#     test_merge_value(CountMap(SortedDict{Int, Int}()), rand(1:4, 100), rand(5:123, 50), ==)
...
#     test_merge_value(CovMatrix(), x, x2)
# end
# #-----------------------------------------------------------------------# CStat 
# @testset "CStat" begin 
#     data = y + y2 * im 
...
#     test_merge_value(CStat(Mean()), y, y2)
#     test_merge_value(CStat(Mean()), data, data2)
# end
# #-----------------------------------------------------------------------# Diff 
# @testset "Diff" begin 
...
#     test_merge_value(Extrema(), y, y2, ==)
# end
# #-----------------------------------------------------------------------# Fit[Dist]
# @testset "Fit[Dist]" begin 
# @testset "FitBeta" begin 
#     test_merge_value(FitBeta(), rand(10), rand(10))
...
#     test_merge_value(FitMultinomial(3), rand(1:4, 10, 3), rand(2:7, 11, 3))
# end
# @testset "FitMvNormal" begin 
#     test_merge_value(FitMvNormal(2), [y y2], [y2 y])
...
#     test_merge_value(FTSeries(Mean(), Variance(); transform = abs), y, y2, (a,b)->≈(value(a),value(b)))
...
#     test_merge_value([Mean() Variance() Sum() Moments() Mean()], x, x2, (a,b) -> all(value.(a) .≈ value.(b)))
#     test_merge_value(5Mean(), x, x2, (a,b) -> all(value.(a) .≈ value.(b)))
#     test_merge_value(5Variance(), x, x2, (a,b) -> all(value.(a) .≈ value.(b)))
#     @test 5Mean() == 5Mean()
...
@testset "FixedBins" begin
    test_merge_value(Hist(-5:.1:5), y, y2)
...
@testset "AdaptiveBins" begin 
    ...
    test_merge_value(Hist(2000), y, y2)
    test_merge_value(Hist(1), y, y2)
    test_merge_value(Hist(2000, Float32), Float32.(y), Float32.(y2))
    test_merge_value(Hist(Float32, 2000), Float32.(y), Float32.(y2))
...
#     test_merge_value(HyperLogLog(4), y, y2)
# end
...
#     test_merge_value(LinReg(), (x,y), (x2,y2))
#     # ridge 
#     o = fit!(LinReg(), (x,y))
#     @test coef(o) ≈ x \ y
...
#     test_merge_value(LinRegBuilder(), [x y], [x2 y2])
...
@testset "Mean" begin 
    ...
    test_merge_value(Mean(), y, y2)
    test_merge_stat(Mean(), y, y2)
end
# #-----------------------------------------------------------------------# Moments
...
#     test_merge_value(Moments(), y, y2)
# end
#-----------------------------------------------------------------------# NBClassifier
@testset "NBClassifier" begin 
    X, Y = randn(1000, 5), rand(Bool, 1000)
    X2, Y2 = randn(1000, 5), rand(Bool, 1000)
    test_merge_stat(NBClassifier(5, Bool; stat=FitNormal()), (X,Y), (X2,Y2))
    # Y[1] = Y2[1] = true
    # o = fit!(NBClassifier(5, Bool), (X, Y))
    # o2 = fit!(NBClassifier(5, Bool), (X2, Y2))
    # merge!(o, o2)
    # fit!(o2, (X, Y))
    # for (a,b) in zip(o.init, o2.init)
    #     @test a == b
    # end
    # @test sort(value(o)[1]) == sort(value(o2)[1])
    # for i in 1:5, j in 1:2 
    #     @test ≈(value(o.d[true][i])[j], value(o2.d[true][i])[j], atol=.1)
    # end
end
#-----------------------------------------------------------------------# OrderStats
@testset "OrderStats" begin 
    test_merge_value(OrderStats(100), y, y2)
...
@testset "Quantile/PQuantile" begin 
    ...
    for o in [
        ...
        test_merge_value(copy(o), data, data2, atol = .5)
    end
    for τi in τ
...
@testset "Series" begin 
    test_merge_value(Series(Mean(), Variance()), y, y2, (a,b) -> ≈(value(a), value(b)))
...
@testset "Variance" begin 
    ...
    test_merge_value(Variance(), y, y2)
end
