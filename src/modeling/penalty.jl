...
_j(p::L2Penalty, β::VecF) = 0.5 * sumabs2(β)
...
_j(p::L1Penalty, β::VecF) = sumabs(β)
...
_j(p::ElasticNetPenalty, β::VecF) = p.α * sumabs(β) + (1. - p.α) * 0.5 * sumabs2(β)
...
# function _j(p::SCADPenalty, β::VecF)
#     val = 0.0
#     for j in 1:length(β)
#         βj = abs(β[j])
#         if βj < p.λ
