...
"""
abstract type AbstractSeries{I} end
#----------------------------------------------------------------# AbstactSeries methods
...
Base.ndims{N}(o::AbstractSeries{N}) = N
...
show_series(io, o::AbstractSeries) = print(io)

#----------------------------------------------------------------# AbstactSeries{0} fit!
function fit!(o::AbstractSeries{0}, y::Real, γ::Float64 = nextweight(o))
    updatecounter!(o)
    singleton_update!(o, y, γ)
    o
end
function fit!(o::AbstractSeries{0}, y::AVec)
    for yi in y
        fit!(o, yi)
    end
    o
end
function fit!(o::AbstractSeries{0}, y::AVec, γ::Float64)
    for yi in y
        fit!(o, yi, γ)
    end
    o
end
function fit!(o::AbstractSeries{0}, y::AVec, γ::AVecF)
...
function fit!(o::AbstractSeries{0}, y::AVec, b::Integer)
    maprows(b, y) do yi
        bi = length(yi)
        γ = weight!(o, bi)
        batch_update!(o, yi, γ)
...
#----------------------------------------------------------------# AbstactSeries{1} fit!
function fit!(o::AbstractSeries{1}, y::AVec, γ::Float64 = nextweight(o))
    updatecounter!(o)
    singleton_update!(o, y, γ)
    o
end
function fit!(o::AbstractSeries{1}, y::AMat)
    for i in 1:size(y, 1)
        fit!(o, view(y, i, :))
    end
    o
end
function fit!(o::AbstractSeries{1}, y::AMat, γ::Float64)
    for i in 1:size(y, 1)
        fit!(o, view(y, i, :), γ)
    end
    o
end
function fit!(o::AbstractSeries{1}, y::AMat, γ::AVecF)
    for i in 1:size(y, 1)
        fit!(o, view(y, i, :), γ[i])
    end
    o
end
function fit!(o::AbstractSeries{1}, y::AMat, b::Integer)
    maprows(b, y) do yi
        bi = size(yi, 1)
        γ = weight!(o, bi)
        batch_update!(o, yi, γ)
...
for (T, I) in [(:Series, 0), (:MvSeries, 1)]
    @eval begin
        ...
        stats(o::$T) = o.stats
        stats(o::$T, i::Integer) = stats(o)[i]
        value(o::$T) = map(value, o.stats)
        value(o::$T, i::Integer) = value(stats(o, i))
        ...
        function Base.merge!{T <: $T}(o::T, o2::T, method::Symbol = :append)
            n2 = nobs(o2)
            n2 == 0 && return o
            p = length(o.stats)
            for i in 1:p
