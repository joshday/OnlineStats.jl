...
function FitCauchy(y::VecF, wgt::StochasticWeighting = StochasticWeighting(); start = zeros(3))
    o = FitCauchy(wgt, start = start)
    update!(o, y)
    o
end

function onlinefit(::Type{Cauchy}, y::VecF, wgt::StochasticWeighting = StochasticWeighting(); start = zeros(3))
    FitCauchy(y, wgt, start = start)
end



function update!(o::FitCauchy, y::Float64)
    update!(o.q, y)
    s = state(o.q)[1]
