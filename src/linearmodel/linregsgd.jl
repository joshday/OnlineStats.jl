...
function LinRegSGD(x::AMatF, y::AVecF, wgt::StochasticWeighting = StochasticWeighting(.7),
...
function LinRegSGD(x::AVecF, y::Float64, wgt::StochasticWeighting = StochasticWeighting(.7),
...
LinRegSGD(p::Integer, wgt::StochasticWeighting = StochasticWeighting(.7), start = zeros(p)) =
...
#---------------------------------------------------------------------# update!
function update!(o::LinRegSGD, x::AVecF, y::Float64)
    λ = weight(o)
    o.β += λ * (y - sum(x .* o.β)) * x
    o.n += 1
end
...
#------------------------------------------------------------------------# Base
function StatsBase.predict(o::LinRegSGD, x::AMat)
    β = coef(o)
    β[1] + x * β[2:end]
end
