...
function FitCauchy(wgt::LearningRate = LearningRate(r = .6); start = zeros(3))
    FitCauchy(Dist.Cauchy(), QuantileMM(wgt, start = start), 0, wgt)
end
...
function distributionfit(::Type{Dist.Cauchy}, y::AVecF, wgt::LearningRate = LearningRate(r = .6); start = zeros(3))
...
function update!(o::FitCauchy, y::Real)
    update!(o.q, y)
    s = state(o.q)[1]
    o.d = Dist.Cauchy(s[2], max(0.5 * (s[3] - s[1]), 1e-10))
    o.n += 1
end
