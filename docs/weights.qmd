...
The following built-in weight types are available:

- **`EqualWeight`** — Equal weight for all observations (default). Equivalent to `weight = inv`.
- **`ExponentialWeight`** — Exponentially decreasing weights.
- **`LearningRate`** — Weight at step $t$ is $1/t^r$ for a given rate $r$.
- **`LearningRate2`** — Weight at step $t$ is $1/(1 + r \cdot t)$ for a given rate $r$.
- **`HarmonicWeight`** — Harmonic series weighting.
- **`McclainWeight`** — Weight from McClain's formula.
...
<br>
<img src="https://user-images.githubusercontent.com/8075494/57347308-d4d27d00-711f-11e9-8fbe-fc4523b96b48.gif" style="width:400px">
